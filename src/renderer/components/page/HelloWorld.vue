            ...
            <div
              ...
              <div class="px-3 pt-2 pb-3 text-center text-xs text-black/50 dark:text-white/50 md:px-4 md:pt-3 md:pb-6">
                <!-- <a href="https://gitee.com/MIEAPP/chatai-vue" target="_blank" rel="noreferrer"
                  ...
                  <button v-if="!popupShow" @click="showUploadResume">
                    <a-icon type="file-pdf" :style="{ fontSize: '32px' }" />
                  </button>     
              </div>
            </div>
          </main>
              ...
              <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div v-if="popupShow"
                  ...
                  <div class="prose dark:prose-invert">
                    <div class="mb-5">
                      <h2 class="!mt-4 font-normal !mb-2"><b>简历上传</b></h2>
                    </div>
                    ...
                    <div class="flex gap-4 flex-col text-sm">
                      <div class="flex p-4 bg-gray-50 dark:bg-white/5 rounded-md items-center gap-4 min-h-[71px]">
                        ...
                        <div class="flex-1 leading-5">
                          <a-input ref="userNameInput" v-model="jobFileName" placeholder="招聘者姓名">
                            <a-icon slot="prefix" type="user" />
                            <a-tooltip slot="suffix" title="输入要上传的简历中的招聘者姓名">
                    ...
                    <div class="confirmBtn">
                      <div class="confirmBtnToSuccess">
                        <button v-if="finshUploadResume" @click="closePopup" class="btn flex justify-center gap-2 btn-primary ml-auto">已完成上传
                        </button>
                      </div>
                      <div class="confirmBtnToDone">
                        <button @click="closePopup" class="btn flex justify-center gap-2 btn-primary ml-auto">取消
                        </button>
                      </div>
                    </div>
                  </div>
...
const renderer = {
  ...
  paragraph(text) {
    return `<p style="white-space:pre-wrap;">${text}</p><br>`
  }
};
marked.use({ renderer });
...
export default {
  data() {
    return {
      ...
      rsource: undefined,
      tsource: undefined,
      jobFileName: "", // 简历对应的招聘者姓名
      fileName: "", // 简历文件名字
      finshUploadResume: false, // 完成上传简历
      pageNum: 10,
      page: 1,
      jobObjectOrder:{
        "company_name": "公司名称：",
  ...
  methods: {
    ...
    async send() {
      ...
      var that = this;
      if (new RegExp('解析简历', 'g').test(chatMsg)) {
        // 匹配到出现有'解析简历'的字样，调接口将符合的岗位导出来
        const analysisResumeData = {
          userId: localStorage.getItem('userId'),
          workername: this.jobFileName,
          pageNum: this.pageNum,
          page: this.page,
        }
        const res = await this.$request.post(this.$API.analysisResume, analysisResumeData);
        console.log(res.data);

        let conv = that.conversation[that.conversation.length - 1];
        conv["loading"] = false;
        that.convLoading = false;

        that.handleScrollBottom();
        var content = that.handleResumeConv(res.data);// 处理推荐的职位信息
        console.log(content);
    ...
    beforeUpload(file) {
      ...
      const fileFormat = this.fileName.substring(this.fileName.lastIndexOf('.')+1);
      if(fileFormat!=="pdf") {
        alert("请上传pdf格式文件")
      }
      const fileSize = file.size;
      if (fileSize > 50*1024*1024) {
        alert("上传的文件超过50MB超过上限")
      }
      this.upload(file);
    },
    upload(file) {
      ...
      formData.append('workername', this.jobFileName);
      let config = {
        headers: {
          'Content-Type': 'multipart/form-data',
        }
      }
      this.$request.post(this.$API.uploadResume, formData, config).then((res) =>{
        if(res.data.uploadcode === 40100) {
          this.finshUploadResume = true;
        }
      })
    ...
    // 处理推荐的职位信息
    handleResumeConv(data) {
      // 1、遍历查出的job数据
      let res = '经ai分析，匹配到的职位详情如下：';
      for (let i = 0; i < data.length; i ++) {
        let jobAllInfo = data[i]; // job所有信息，包含匹配规则
        let jobInfo = jobAllInfo["job"]; // 仅job信息
        const num = i+1;
        res += '\n' + '推荐第' + num + '个职位：' + jobInfo["job"];
        // 2、将job数据按格式洗出来保存到res中
        for (const key in jobInfo) {
          if (Object.hasOwnProperty.call(jobInfo, key)) {
            const jobData = jobInfo[key];
            if(jobData && Object.hasOwnProperty.call(this.jobObjectOrder, key)) {
              res += '\n' + this.jobObjectOrder[key] + jobData;
            }
          }
        }
        // 3、将job的匹配规则洗出来，保存到res中
        res += '\n' + '该职位匹配规则：' + '简历中出现了 “' + jobAllInfo["name"] + '” 关键字与职位相匹配，共出现' + jobAllInfo["num"] + '次！' + '\n';
      }
      return res;
    }
  },
